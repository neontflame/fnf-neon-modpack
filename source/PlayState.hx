...
class PlayState extends MusicBeatState
{
	...
	private var vocalsOPP:FlxSound;
	...
	private var songTimerBarBG:FlxSprite;
	private var songTimerBar:FlxBar;
	...
	// stage sprites
	// comment out everything just in case something goes to shit
...
//		FlxG.sound.cache(Paths.voices(PlayState.SONG.song));
		FlxG.sound.cache(Paths.voicesOPP(PlayState.SONG.song));	
		FlxG.sound.cache(Paths.voicesBF(PlayState.SONG.song));
	...
	// stage shit starts here
		...
		else if (stageCheck == 'template')
		{
			curStage = "template";

			defaultCamZoom = 0.75;
			
			var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image("stages/template/stageback"));
			...
			var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image("stages/template/stagefront"));
		...
		else if (stageCheck == 'ntfstage')
		{
			curStage = "ntfstage";

			defaultCamZoom = 0.8;
			
			var bg:FlxSprite = new FlxSprite(-400, -200).loadGraphic(Paths.image("stages/ntfstage/bg"));
			// bg.setGraphicSize(Std.int(bg.width * 2.5));
			// bg.updateHitbox();
			bg.antialiasing = true;
			...
			var front:FlxSprite = new FlxSprite(-500, 450).loadGraphic(Paths.image("stages/ntfstage/front"));
			front.updateHitbox();
...
// stage shit ends here i suppose
...
// end character shit
		...
		if(Config.downscroll){
			strumLine = new FlxSprite(0, 570).makeGraphic(FlxG.width, 10);
				...
				if(Config.songTimerType != 2){
					strumLine = new FlxSprite(0, 40).makeGraphic(FlxG.width, 10);
				} else {
		...
		// Add watermark (taken from Kade Engine and then modified a bit)
		kadeEngineWatermark = new FlxText(4, (Config.downscroll ? healthBarBG.y - 50 : healthBarBG.y + 50), 0, SONG.song + " - TNM 1.0", 16);
		kadeEngineWatermark.setFormat(Paths.font("vcr"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		kadeEngineWatermark.scrollFactor.set();

		// hell yeah score text
		
		if(!Config.vanillaUi){
		// IF NOT VANILLA UI THEN DO THINGS
			if(Config.downscroll) {
		...
		// IF VANILLA UI THEN DO OTHER THINGS
		// idfk how to haxe
		scoreTxt = new FlxText(healthBarBG.x + healthBarBG.width - 191, healthBarBG.y + 30, 800, "", 22);
		scoreTxt.setFormat(Paths.font("vcr"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		}
		...
		// song timer thing
			songTimerBarBG = new FlxSprite(0, Config.downscroll ? FlxG.height * 0.95 : FlxG.height * 0.02).loadGraphic(Paths.image('healthBar'));
			songTimerBarBG.screenCenter(X);
			songTimerBarBG.scrollFactor.set();

			songTimerBar = new FlxBar(songTimerBarBG.x + 4, songTimerBarBG.y + 4, LEFT_TO_RIGHT, Std.int(songTimerBarBG.width - 8), Std.int(songTimerBarBG.height - 8), this,'songPercent', 0, 1);
			songTimerBar.scrollFactor.set();
			songTimerBar.createFilledBar(0xFF000000, 0xFF66FF33);
			// songTimerBar
		
			songTimerTxt = new FlxText(songTimerBarBG.x+2, songTimerBarBG.y-2, 800, "", 22);
			songTimerTxt.setFormat(Paths.font("vcr"), 16, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(Config.songTimerType < 2){
			...
			if(Config.songTimerType == 0){
				songTimerTxt.text = SONG.song;
			}
		}
		...
		songTimerBar.cameras = [camHUD];
		songTimerBarBG.cameras = [camHUD];
		songTimerTxt.cameras = [camHUD];
	...
	function coioIntro(?dialogueBox:DialogueBox):Void
	{
		camHUD.visible = false;
		inCutscene = true;
		dad.playAnim('cut', true);
		camFollow.y += 200;
		camFollow.x -= 40;
		
		new FlxTimer().start(1.2, function(awesomeTimer:FlxTimer)
			{
			camHUD.visible = true;
			inCutscene = false;
			startCountdown();
	...
	function startSong():Void
	{
		...
		vocalsOPP.play();
		trace('vocals opp play');
		vocalsBF.play();
		trace('vocals bf play');
		
		if(sectionStart){
			FlxG.sound.music.time = sectionStartTime;
			Conductor.songPosition = sectionStartTime;
			vocalsOPP.time = sectionStartTime;
			vocalsBF.time = sectionStartTime;
		}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
		{
			vocalsOPP = new FlxSound().loadEmbedded(Paths.voicesOPP(PlayState.SONG.song));
			FlxG.sound.list.add(vocalsOPP);
			trace('vocals opp load embedded');
			vocalsBF = new FlxSound().loadEmbedded(Paths.voicesBF(PlayState.SONG.song));
			FlxG.sound.list.add(vocalsBF);
			trace('vocals bf load embedded');
		}
		else
		{
			vocalsOPP = new FlxSound();
			FlxG.sound.list.add(vocalsOPP);
			trace('vocals opp there aint shit');
			vocalsBF = new FlxSound();
			FlxG.sound.list.add(vocalsBF);
			trace('vocals bf there aint shit');
		}
		
	//	FlxG.sound.list.add(vocalsOPP);
	//	trace('flxg vocalsopp added to list');
	//	FlxG.sound.list.add(vocalsBF);
	//	trace('flxg vocalsbf added to list');
	//	lotta dumb shit all around
	...
	override public function update(elapsed:Float)
	{
		...
		else
		{
			...
			if (!paused)
			{
				...
				if(updateTime) {
					var curTime:Float = Conductor.songPosition - Config.offset;
	...
	override function stepHit()
	{
		if (SONG.needsVoices)
		{
			if (vocalsOPP.time > Conductor.songPosition + 20 || vocalsOPP.time < Conductor.songPosition - 20)
			{
				resyncVocals();
			}
			if (vocalsBF.time > Conductor.songPosition + 20 || vocalsBF.time < Conductor.songPosition - 20)
			{
				resyncVocals();
			}
		}
