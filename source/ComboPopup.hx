...
class ComboPopup extends FlxSpriteGroup
{
	...
	**/
	public function comboPopup(_combo:Int):Void{
		...
		for(i in 0...combo.length){
			...
			var digit = new FlxSprite(numberPosition[X] + (numberInfo[WIDTH] * numberScale * 0.8 * i), numberPosition[Y]).loadGraphic(numberInfo[GRAPHIC], true, numberInfo[WIDTH], numberInfo[HEIGHT]);
			...
			FlxTween.tween(digit, {alpha: 0}, 0.3, {
	...
	**/
	public function ratingPopup(_rating:String):Void{
		...
		FlxTween.tween(ratingSprite, {alpha: 0}, 0.3, {
			onComplete: function(tween:FlxTween){
	...
	**/
	public function breakPopup():Void{
		...
		FlxTween.tween(breakSprite, {alpha: 0}, 0.3, {
			onComplete: function(tween:FlxTween){
				breakSprite.destroy();
			},
