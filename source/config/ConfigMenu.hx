...
class ConfigMenu extends MusicBeatState
{
	...
	var scheme:Int;
	var dimValue:Int;
	var songTimerTypes:Array<String> = ["song name", "no text", "off"];
	var songTimerValue:Int;
	var vanillaUiValue:Bool;
	...
	final settingDesc:Array<String> = [
									...
									"TEMP",
									"Change key binds.",
									"Shows how much of the song is left.",
									"Just like the base game.\n(Works best with accuracy display and song timer disabled!)"
									];
	...
	{	
		...
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('menuBGOptions'));
		bg.scrollFactor.x = 0;
		bg.scrollFactor.y = 0;
		...
		if (tweeningallowed)
		{ 
			tweeningallowed = false;
			FlxTween.tween(bg, { x: -128 }, 2, {type: FlxTween.LOOPING});
			tweeningallowed = true;
			bg.setPosition(0, 0);
		}
		...
		songTimerValue = Config.songTimerType;
		vanillaUiValue = Config.vanillaUi;
		
		var tex = Paths.getSparrowAtlas('FNF_main_menu_assets_blue');
		var optionTitle:FlxSprite = new FlxSprite(0, -20);
		optionTitle.frames = tex;
		optionTitle.setGraphicSize(Std.int(optionTitle.width * 0.85));	
		...
		configText = new FlxText(0, 155, 1280, "", 42);
		...
		configText.setFormat(Paths.font("trebuc", "ttf"), 26, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.SHADOW, FlxColor.BLACK);
		configText.borderSize = 3;
		configText.borderQuality = 1;
		
		descText = new FlxText(320, 642, 640, "", 20);
			...
			if (controls.UP_P)
				...
				switch(configSelected){
					...
					case 13: //Song Timer
						...
						if (songTimerValue >= songTimerTypes.length)
							songTimerValue = 0;
						if (songTimerValue < 0)
							songTimerValue = songTimerTypes.length - 1;
					case 14: //Vanilla UI
						if (controls.RIGHT_P || controls.LEFT_P || controls.ACCEPT) {
							FlxG.sound.play(Paths.sound('scrollMenu'));
							vanillaUiValue = !vanillaUiValue;
						}
			}
		}
	...
	function getSetting(r:Int):String{
		...
		switch(r){
			...
			case 13: return ": " + songTimerTypes[songTimerValue];
			case 14: return ": " + genericOnOff[vanillaUiValue?0:1];
		}
	...
	function secretPresetTest(_combo:Array<String>):Void{
		...
		switch(combo){
			...
			case "KADE":
				Config.write(offsetValue, "complex", 5, 5, 1, downValue, false, 2, noCapValue, scheme, dimValue, songTimerValue, vanillaUiValue);
				exit();
			case "ROZE":
				Config.write(offsetValue, "simple", 1, 1, 0, true, true, 0, noCapValue, scheme, dimValue, songTimerValue, vanillaUiValue);
				exit();
			case "CVAL":
				Config.write(offsetValue, "simple", 1, 1, comboValue, false, glowValue, 1, noCapValue, scheme, dimValue, songTimerValue, vanillaUiValue);
				exit();
	...
	function writeToConfig(){
		Config.write(offsetValue, accuracyType, healthValue / 10.0, healthDrainValue / 10.0, comboValue, downValue, glowValue, randomTapValue, noCapValue, scheme, dimValue, songTimerValue, vanillaUiValue);
	}
