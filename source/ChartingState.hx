...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addToolsUI():Void
		{
			...
			var check_mute_vox_opp = new FlxUICheckBox(10, 90, null, null, "Mute Opponent Vocals (in editor)", 100);
			check_mute_vox_opp.checked = false;
			check_mute_vox_opp.callback = function()
			{
				var vol:Float = 1;
	
				if (check_mute_vox_opp.checked)
					vol = 0;
	
				vocalsOpp.volume = vol;
			};

			var check_mute_vox_bf = new FlxUICheckBox(10, 110, null, null, "Mute BF Vocals (in editor)", 100);
			check_mute_vox_bf.checked = false;
			check_mute_vox_bf.callback = function()
			{
				var vol:Float = 1;
	
				if (check_mute_vox_bf.checked)
					vol = 0;
	
				vocalsBf.volume = vol;
			};
			
			var lilBuddiesBox = new FlxUICheckBox(10, 130, null, null, "Lil' Buddies", 100);
		...
		//	halfSpeedCheck.checked = false;
			...
			tab_group_tools.add(check_mute_vox_opp);
			tab_group_tools.add(check_mute_vox_bf);
	...
	function loadSong(daSong:String):Void
	{
		...
		vocalsOpp = new FlxSound().loadEmbedded(Paths.voicesOPP(daSong));
		FlxG.sound.list.add(vocalsOpp);
		vocalsBf = new FlxSound().loadEmbedded(Paths.voicesBF(daSong));
		...
		vocalsOpp.pause();
		vocalsBf.pause();
		vocalsOpp.time = FlxG.sound.music.time;
